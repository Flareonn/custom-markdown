...
const turndownService = new TurndownService({
  ...
  blankReplacement: (content, node) => {
    return node.isBlock ? "\xa0  \n" : "";
  },
});
turndownService.use([gfm, todoList, breakLines]);
...
function todoList(turndownService: TurndownService): void {
  turndownService.addRule("taskListItems", {
    ...
    replacement(_: unknown, node: any) {
      return (node.checked ? "[x]" : "[ ]") + " ";
    },
  });
}

function breakLines(turndownService: TurndownService): void {
  turndownService.addRule("p-break-line", {
    filter: "p",
    replacement(content) {
      return content + "  \n";
    },
  });
  turndownService.addRule("br-break-line", {
    filter: "br",
    replacement() {
      return "\xa0\n";
    },
